...
class Problem(Slide):
    def construct(self):
        ...
        ).next_to(ProblemStatementTitle, DOWN)
        HeatEQN_2 = MathTex(r"\frac{\partial u}{\partial t}",
                            r"-",
                            r"\frac{\partial ^{2}u}{\partial x^{2}}",
                            r"-",
                            r"\frac{\partial ^{2}u}{\partial y^{2}}"
        ).next_to(ProblemStatementTitle, DOWN)
        EqualsZero = MathTex(r"= 0").next_to(HeatEQN_2, RIGHT)

        Solution = MathTex(r"u(x,y,t) = \text{Temperature}_{x,y,t}").next_to(HeatEQN_2, DOWN).scale(0.8).shift(RIGHT * 0.7)

        InitialCondition = MathTex(r"u(x,y,0) = \frac{1}{2\pi \sigma^{2}}e^{-\frac{(x-x_{0})^{2} + (y-y_{0})^{2}}{2\sigma^{2}}}").next_to(Solution, DOWN).shift(RIGHT * 0.55).scale(0.8)
        InitialConditionImage = ImageMobject("initial_conditions.png").scale(1.5).to_edge(RIGHT)

        BoundaryConditions0 = MathTex(r"u(0,y,t) = u(x,0,t) = 0").next_to(InitialCondition, DOWN).scale(0.8)
        BoundaryConditions1 = MathTex(r"u(1,y,t) = u(x,1,t) = 0").next_to(BoundaryConditions0, DOWN).scale(0.8)
        ...
        self.next_slide()
        self.play(Transform(HeatEQN_1, HeatEQN_2), FadeIn(EqualsZero))
        self.next_slide()
...
class ChainRule(Slide):
    def construct(self):
        ...
        self.start_loop()
        self.wait(0.5)
        self.play(x_parameter.animate.set_value(2.5), runtime=2, rate_func=rate_functions.linear)
        self.wait(0.5)
        self.end_loop()
